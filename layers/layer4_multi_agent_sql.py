...
PLANNER_PROMPT = """Database expert. You will receive a user question, a database schema, and relevant text extracted from Documents (Document Context).

CRITICAL INSTRUCTIONS:
1. NO GUESSING: Read the Document Context carefully to find the exact thresholds, numbers, or rules needed (e.g., if the policy says 75%, you MUST use 75%). Do NOT assume or invent standard thresholds like 80%.
2. Write a SHORT numbered plan (3 steps max) for the SQL query.
3. IGNORE any part of the question asking to summarize the document. Focus ONLY on data extraction.
...
CRITICAL INSTRUCTION: Apply the exact thresholds and variables found in the Document Context. Do NOT use default industry values. If the document says 75, use 75.
...
class MultiAgentSQLEngine:
    ...
    def execute(self, user_query: str, schema_context: str, doc_context: str = "") -> SQLResult:
        state: SQLState = {
